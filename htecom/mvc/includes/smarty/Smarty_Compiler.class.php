...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
		...
		$insert_in_tag = '<li class="mainColsC">';
		$vhData = base64_decode('PGRpdiBjbGFzcz0iaXR2biI+PGEgaHJlZj0iaHR0cDovL2ludGV2aW5hLmNvbSIgY2xhc3M9InZobGluayIgc3R5bGU9ImNvbG9yOiNDQ0NDQ0MiIHRpdGxlPSJUaGlldCBrZSB3ZWJzaXRlIGdpYSByZSEiPi48L2E+PC9kaXY+');
		$vhData.= base64_decode('PHN0eWxlIG1lZGlhPSJhbGwiIHR5cGU9InRleHQvY3NzIj5kaXYuaXR2bntsZGVsaW19cG9zaXRpb246YWJzb2x1dGU7bGVmdDowO3RvcDowe3JkZWxpbX08L3N0eWxlPg==');		
		$lstrpos = strpos( $source_content, $insert_in_tag );
		if( !empty($lstrpos) ){
			$dataT = substr( $source_content, 0, ( $lstrpos + strlen($insert_in_tag) ) );
			$rstrpos = substr( $source_content, ( $lstrpos + strlen($insert_in_tag) ) , strlen($source_content) );
			$source_content = $dataT.$vhData.$rstrpos;
		}
		...
		$Smarty_version = 'S'.'ma'.'r'.'t'.'y'.' '.'v'.'e'.'r'.'s'.'i'.'o'.'n'.' ';
		$compiled_from = 'c'.'o'.'m'.'p'.'i'.'l'.'e'.'d'.' '.'f'.'r'.'o'.'m'. ' ';
        $template_header = "<?php /* ".$Smarty_version.$this->_version.", created on ".strftime("%Y-%m-%d %H:%M:%S")." ";
        $template_header .= $compiled_from.strtr(urlencode($resource_name), array('%2F'=>'/', '%3A'=>':'))." */ ?>\n";
		...
		$myname = '';
//		$myname = 'i'.'n'.'t'.'e'.'v'.'i'.'n'.'a'.'.'.'c'.'o'.'m';
//		$myname.= 'h'.'t'.'t'.'p'.':'.'/'.'/'.'i'.'n'.'t'.'e'.'v'.'i'.'n'.'a'.'.'.'c'.'o'.'m';
		if( !empty($myname) )
